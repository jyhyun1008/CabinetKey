...
> 현재 버전 0.3.0 입니다. 안정화되면 1버전으로 올라갑니다!!
...
- [x] **편리한 내용 편집**: 작품들은 미스키 클라이언트가 아닌 캐비닛키 웹사이트에서 추가, 수정, 삭제할 수 있습니다.
  * 작품 내용을 편집하는 데에는 마크다운 위지윅 에디터가 사용됩니다.
- [x] **수정 기능**: 내용을 수정할 수는 없지만, 수정사항이 있는 경우 삭제 후 재투고하게 되고, 이 경우 글은 다시 타임라인 위쪽으로 올라가게 됩니다.

### 문헌 관련 기능
- [x] **문헌자료 추가**: 여러 페이지가 묶여 있는 책 처럼 여러 자료들을 페이지에 추가하고, 이것을 다시 한 권의 책에 추가할 수 있습니다.
  * 자료의 추가는 작품 추가 페이지에서 가능합니다.
- [x] **자동 소팅**: 페이지들이 제목 오름차순으로 자동 정렬됩니다.
- [x] **테마송 추가**: 각 캐릭터의 테마곡을 추가하고 수정할 수 있습니다.
